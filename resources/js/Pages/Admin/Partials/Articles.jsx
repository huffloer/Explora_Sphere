...
export default function Articles({ auth }) {
    ...
    const { articles, subcategories } = usePage().props;
    const columnnames = [
        "Article ID",
        "Titre",
        "Auteur",
    ...
    const handleSubmit = (e) => {
        ...
        setVisible(false);
        setShowPopUp(true);
        const response = post(route("articles.store"));
        // Handle successful response here, update state as necessary
    ...
    const handleConfirm = () => {
        ...
        if (actionType === "disable") {
            // Inertia.post(route("disablearticle", { articleId: article.id }));
        } else if (actionType === "delete") {
            // Inertia.post(route("deletearticle", { articleId: article.id }));
        }
    };
        ...
        <AdminLayout auth={auth}>
                        ...
                        {articles.map((article) => (
                            ...
                            >
                                <>
                                    ...
                                    <td className=" px-4 py-4 text-left font-semibold text-gray-700 text-xs">
                                        {article.title}
                                    </td>
                                    <td className=" px-4 py-4 text-left font-semibold text-gray-700 text-xs">
                                        {article.author}
                                    ...
                                    <td className=" px-4 py-4 text-left font-semibold text-gray-700 text-xs">
                                        {/* {subcategories[article.subcategory_id]['title']} */}
                                        {subcategories[article.subcategory_id]
                                            ? subcategories[article.subcategory_id]['title']
                                            : "error"}
                                    </td>
                                    <td className=" px-4 py-4 text-center font-semibold text-gray-700 text-xs">
                                    ...
                                    <td className=" px-4 py-4 text-center underline font-semibold text-gray-700 text-xs">
                                        <a href={article.url}>
                                            Lien de l'article
                                        </a>
                                    </td>
            ...
            >
                ...
                <div className="max-w-7xl my-6 w-full mx-auto sm:px-6 lg:px-8 space-y-6">
                    ...
                    >
                        <div className="w-full mr-10 flex ">
                            <div className="w-1/2">
                                <div className="my-3 w-full items-baseline rounded-md justify-around">
                                    <InputLabel
                                        className="w-52 block font-medium text-sm rounded-md text-gray-700 dark:text-gray-300"
                                        htmlFor="subcategory"
            ...
            <AddedPopup show={showPopUp} message="Article ajouté" />
            {showModal && (
                <SmallModal
